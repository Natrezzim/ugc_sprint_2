...
class MovieLikesService:
    ...
    def __init__(self, model: MovieLikeSchema, collection: AgnosticCollection):
        self.model = model
        self.collection = collection

    async def _get_movie_data(self, movie_filter: dict) -> Optional[dict]:
    ...
    async def add_like_dislike(self, user_id: UUID, movie_id: UUID, like: bool) -> MovieLikeSchema:
        ...
        if movie := await self.collection.find_one(movie_filter):
            ...
            like_by = [user_id] if like else []
            dislike_by = [user_id] if not like else []
            rating = get_rating(like_by, dislike_by)
            movie_data = {
                "movie_id": movie_id,
                "like_by": like_by,
    ...
    async def remove_like_dislike(self, user_id: UUID, movie_id: UUID, like: bool) -> Optional[MovieLikeSchema]:
        user_id = str(user_id)
        movie_id = str(movie_id)
